...
struct MasterMatrixW::Impl
{
    ...
    ExtendedCanonicalizer echelonizer; ///< The echelonizer of matrix Wx = [Ax; Jx]
...
MasterMatrixW::MasterMatrixW(Index nx, Index np, Index ny, Index nz, MatrixConstRef Ax, MatrixConstRef Ap)
...
MasterMatrixW::MasterMatrixW(const MasterMatrixW& other)
...
MasterMatrixW::~MasterMatrixW()
{}

auto MasterMatrixW::update(MatrixConstRef Jx, MatrixConstRef Jp, VectorConstRef weights) -> void
{
    pimpl->update(Jx, Jp, weights);
}

auto MasterMatrixW::echelonForm() const -> EchelonForm
{
    return pimpl->echelonForm();
}
