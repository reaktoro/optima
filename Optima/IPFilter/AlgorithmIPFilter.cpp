...
double AlgorithmIPFilter::State::CalculateLargestDelta() const
{
    ...
    const double tau = 1.0 - std::min(0.01, 100.0*mu*mu);

    return tau*delta;
}

void AlgorithmIPFilter::State::SearchDeltaNeighborhood()
{
...
double AlgorithmIPFilter::State::CalculateRhoRestoration() const
{
    const double theta2_old = (thh_old*thh_old + thc_old*thc_old)/2.0;
    const double theta2 = (thh*thh + thc*thc)/2.0;

    const auto& aux1 = h_old.grad.transpose()*h_old.func;
    const auto& aux2 = (x_old.array() * z_old.array() - mu_old).matrix();

    const double ared = theta2_old - theta2;
    const double pred = -(aux1 + z_old.cwiseProduct(aux2)).dot(snx) - x_old.cwiseProduct(aux2).dot(snz);

    return ared/pred;
}

void AlgorithmIPFilter::State::SearchDeltaTrialTests()
{
    if(theta <= delta * std::min(params.gamma1, params.gamma2 * std::pow(delta, params.beta)))
    {
        while(delta > params.delta_min)
        {
...
void AlgorithmIPFilter::State::SearchDeltaRestoration()
{
    while(delta > params.delta_min)
    {
        const double rho = CalculateRhoRestoration();
...
void AlgorithmIPFilter::State::UpdateNormalTangentialSteps()
{
    // Define some auxiliary variables
    ...
    rhs_n.segment(0, n) = - z_old + (mu_old/x_old.array()).matrix();
    rhs_n.segment(n, m) = - h_old.func;

    // Assemble the tangential rhs vector of the linear system
    rhs_t.segment(0, n) = - f_old.grad - h_old.grad.transpose()*y_old + z_old - ((1 - sigma)*mu_old/x.array()).matrix();
    rhs_t.segment(n, m) = - VectorXd::Zero(m);

    // Calculate the LU decomposition of the coefficient matrix
    lu.compute(lhs);

    // Calculate the normal and tangential components of the step
    un = lu.solve(rhs_n);
    ut = lu.solve(rhs_t);

    // Extract the x and y components of the normal and tangential steps
    snx = un.segment(0, n);
    sny = un.segment(n, m);
    stx = ut.segment(0, n);
    sty = ut.segment(n, m);
