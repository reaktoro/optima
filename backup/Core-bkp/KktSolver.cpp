...
auto KktSolverRangespaceDiagonal::decompose(const KktMatrix& lhs) -> void
{
    ...
    D1.noalias() = rows(D, ipivot);
    D2.noalias() = rows(D, inonpivot);
    A1.noalias() = cols(A, ipivot);
    A2.noalias() = cols(A, inonpivot);
...
auto KktSolverRangespaceDiagonal::solve(const KktVector& rhs, KktSolution& sol) -> void
{
    ...
    r.noalias() = a + c/X;
    a1.noalias() = rows(r, ipivot);
    a2.noalias() = rows(r, inonpivot);
